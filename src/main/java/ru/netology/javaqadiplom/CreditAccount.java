...
public class CreditAccount extends Account {
    ...
    public CreditAccount(int initialBalance, int creditLimit, int rate) throws IllegalArgumentException {
        if (rate < 0) { // Добавлены поля для кредитного лимита и
            throw new IllegalArgumentException(                    //     и текущего баланса
        ...
        }                     // Добавлены исключения для лимита и баланса
    ...
    public boolean pay(int amount) {
        ...
        if (balance >= -creditLimit) {   //Больше или равен, а не простобольше
            return true;
        } else {
            balance += amount;           //В случае false, а не true
            return false;
        }
    }
    ...
    public boolean add(int amount) {
        ...
        balance += amount;  //Прибавляется, а не приравнивается
        return true;
    }
